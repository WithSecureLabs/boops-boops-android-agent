...
import com.boops.jdiesel.api.connectors.ServerSocketFactory;
import com.boops.jdiesel.api.connectors.Connector.Status;
import com.boops.jdiesel.api.transport.SocketTransport;
...
public class Server extends Link {
	...
	public Server(com.boops.jdiesel.api.connectors.Server parameters, DeviceInfo device_info) {
	...
	public void resetConnection() {
		this.parameters.setStatus(com.boops.jdiesel.api.connectors.Server.Status.CONNECTING);
	...
	public void run() {
		...
		while(this.running) {
			try {
				if(this.connection == null) {
					...
					this.log(LogMessage.INFO, "Attempting to bind to port " + ((com.boops.jdiesel.api.connectors.Server)this.parameters).getPort() + "...");
					this.server_socket = new ServerSocketFactory().createSocket((com.boops.jdiesel.api.connectors.Server)this.parameters);
					
					this.log(LogMessage.INFO, "Waiting for connections...");
					Socket socket = this.server_socket.accept();
					
					if(socket != null) {
						this.parameters.setStatus(com.boops.jdiesel.api.connectors.Server.Status.ONLINE);
						
						this.log(LogMessage.INFO, "Accepted connection...");
						
						this.log(LogMessage.INFO, "Starting boopsboops thread...");
		...
		this.parameters.setStatus(com.boops.jdiesel.api.connectors.Server.Status.OFFLINE);
	}
